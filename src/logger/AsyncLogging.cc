...
{
    // append会在多个线程中调用，也就是多个线程会同时向currentBuffer_中写数据，因此要加锁
    ...
    else
    {
        ...
        {
            // 如果前端写入速度太快，一下就把两块缓冲区用完了，那么就分配一块新的buffer
            // 作为当前的缓冲区，这种情况很少见
...
void AsyncLogging::threadFunc()
{
    ...
    {
        {
            // 条件变量需要用 unique_lock 不可以用 lock_guard
            // 互斥锁保护，这样别的线程在这段时间就无法向前端buffers_写入数据
            ...
            // 此时正使用得buffer也放入buffers_中（没写完也放进去，避免等待太久才刷新一次）
            buffers_.push_back(std::move(currentBuffer_));
            // 归还正使用缓冲区

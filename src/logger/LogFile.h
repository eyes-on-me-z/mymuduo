...
{
public:
    // threadSafe表示是否需要考虑线程安全，其的作用：同步日志需要锁，因为日志可能来自多个线程
    // 异步日志不需要锁，
    ...
    void append(const char *logline, int len);  // 向file_的缓冲区buffer_中继续添加日志信息
...
private:
    ...
    // 也会在往file_的buffer_写入的次数达到checkEveryN_时，刷新一下（fflush），即把缓冲区的数据存储到文件中。
    const int checkEveryN_; // count_超过checkEveryN_次就fflush一下

    int count_; // 记录往buffer_添加数据的次数

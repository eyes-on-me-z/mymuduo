...
private:
    ...
    ConnectionCallback connectionCallback_; // 有新连接和销毁连接时的回调
    MessageCallback messageCallback_;   // 有读消息时的回调(从Tcp接收缓冲区读取数据到inputBuffer时调用)
    WriteCompleteCallback writeCompleteCallback_;   // 消息发送完成以后的回调
    CloseCallback closeCallback_;
    HighWaterMarkCallback highWaterMarkCallback_;   // 需要发送的数据大于highWaterMark_时调用
